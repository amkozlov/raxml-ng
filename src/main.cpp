...
void autotune_threads(RaxmlInstance &instance) {
  ...
  if (opts.num_threads == 0) {
    ...
    opts.num_threads = opt_rank_threads <= opts.num_threads_max
                         ? opt_rank_threads
                         : (opts.num_threads_max / workers_per_rank) * workers_per_rank;
...
void autotune_start_trees(RaxmlInstance &instance) {
  ...
  if (!diff_pred || difficulty < 0.) {
    ...
    if (opts.constraint_tree_file.empty() || !opts.use_old_constraint) {
      ...
      opts.start_trees[StartingTree::random] = difficulty >= 0.7
                                                 ? 0
...
void predict_msa_difficulty(RaxmlInstance &instance) {
  ...
  if (difficulty >= 0.) {
    ...
    if (difficulty > 0.7) {
      ...
      LOG_INFO <<
          "WARNING! This dataset is considered hard-to-analyze in the sense that the phylogenetic signal is insufficient."
          << endl
          << "Adaptive RAxML-NG will execute a fast heuristic to quickly infer only a few out of the many equally likely topologies."
          << endl
...
void balance_load_coarse(RaxmlInstance &instance, const CheckpointFile &ckpfile) {
  ...
  CoarseAssignmentList start_tree_assign = instance.coarse_load_balancer->get_all_assignments(
    todo_start_trees, num_workers);
...
void thread_infer_ml(RaxmlInstance &instance, CheckpointManager &cm) {
  ...
  for (auto p = 0U; p < treeinfo->pll_treeinfo().partition_count; ++p) {
    LOG_DEBUG << "Part #" << p << ": " << treeinfo->pll_treeinfo().partition_loglh[p] << endl;
  }
...
void thread_infer_bootstrap(RaxmlInstance &instance, CheckpointManager &cm) {
  ...
  unsigned int bs_batch_start = instance.bootstop_checker
                                  ? instance.bootstop_checker->num_bs_trees()
                                  : cm.checkp_file().bs_trees.size();
