...
using ResourceEstimatorFunction = std::function<size_t(const Options &, const PartitionInfo&,
                                                       const ModelDescriptor&,EvaluationPriority)>;
...
class ModelScheduler final {
    public:
        ...
        void update_result(ModelEvaluator &evaluator, const ModelEvaluation &result,
                           bool announce = true, bool write_checkpoint = true);
        ...
        std::vector<std::vector<ModelEvaluation const *>> collect_finished_results_by_partition() const;
        void fetch_global_results();
        const SubstitutionModelDescriptor &get_reference_model();

        static std::vector<size_t> determine_acceptable_thread_counts(size_t total_cores);
        static size_t pick_acceptable_thread_count(const std::vector<size_t> &acceptable_thread_counts,
...
private:
    ...
    std::vector<ModelEvaluator> evaluators;

    /** Keep map that points into the evaluators array to allow addressing by candidate model */
    std::unordered_map<PartitionCandidateModel, uint64_t> evaluator_map;

    Heuristics heuristics;
    DistributedSchedulingImpl distributed_scheduling;

    void _fetch_global_results();
    void _update_result(ModelEvaluator &evaluator, const ModelEvaluation &result,
                        bool announce = true, bool write_checkpoint = true);
    ...
    void read_from_checkpoint(const CheckpointManager &checkpoint_manager);
